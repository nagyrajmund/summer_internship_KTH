...
def predict(GesticulatorModel, speech_file, text_file, gesture_file):
    """ Predict human gesture based on the speech

    Args:
        mean_pose:     mean_pose in the dataset
        GesticulatorModel:       model to evaluate
    ...
    motion_seq = GesticulatorModel.forward(S_tensor.unsqueeze(0), T_tensor.unsqueeze(0)).squeeze()

    # detach
    gestures_norm = np.array(motion_seq.detach().numpy())

    # denormalize
    gestures = gestures_norm * GesticulatorModel.max_val + GesticulatorModel.mean_pose[np.newaxis]
