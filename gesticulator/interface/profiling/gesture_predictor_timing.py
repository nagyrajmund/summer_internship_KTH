...
class GesturePredictorTiming:
    ...
    def predict_gestures(self, audio_fname, text_fname, use_with_dialogflow = False):
        ...
        #START OF BENCHMARK
        print(f"forward pass ({self.NUM_REPEAT} repeats)")
        results = []

        for i in range(self.NUM_REPEAT):
            results.append(time.time())
            # Thing to measure
            self.model.forward(audio, text, use_conditioning=True, motion=None)

        results.append(time.time())
        results = np.diff(results)
        print(f"{results[:2]}{results[-3:]}\n\tmean  {np.mean(results)}\n\tstd:   {np.std(results)}")
        #END OF BENCHMARK

        predicted_motion = self.model.forward(audio, text, use_conditioning=True, motion=None)

        #START OF BENCHMARK
        print(f"Conversion to euler angles ({self.NUM_REPEAT} repeats)")
        results = []

        for i in range(self.NUM_REPEAT):
            results.append(time.time())
            # Thing to measure
            self._convert_to_euler_angles(predicted_motion)
    ...
    def _create_embedding(self, text_dim):
        if text_dim == 773:
            return BertEmbedding(max_seq_length=100, model='bert_12_768_12',dataset_name='book_corpus_wiki_en_cased')
        elif text_dim == 305:
            return FastText()
        else:
    ...
    def _extract_text_features(self, text_fname):
        if isinstance(self.embedding, BertEmbedding):
            # START OF BENCHMARK
            print(f"bert feature extraction ({self.NUM_REPEAT} repeats)")
            results = []

            for i in range(self.NUM_REPEAT):
                results.append(time.time())
                # Thing to measure
                encode_json_transcript_with_bert(text_fname, self.embedding)
            ...
            # START OF BENCHMARK
            print(f"FastText feature extraction ({self.NUM_REPEAT} repeats)")
            results = []

            for i in range(self.NUM_REPEAT):
                results.append(time.time())
                # Thing to measure
                encode_json_transcript_with_fasttext(text_fname, self.embedding)
    ...
    def _extract_features(self, audio_in, text_in, use_with_dialogflow):
        ...
        """
        print(f"Audio feature ({self.feature_type}) extraction  ({self.NUM_REPEAT} repeats)")
        results = []

        for i in range(self.NUM_REPEAT):
            results.append(time.time())
            # Thing to measure
            self._extract_audio_features(audio_in)
        ...
        if use_with_dialogflow:
            ...
            # START BENCHMARK
            print(f"Text timing estimation ({self.NUM_REPEAT} repeats)")
            results = []

            for i in range(self.NUM_REPEAT):
                results.append(time.time())
                # Thing to measure
                self._estimate_word_timings(text_in, total_duration_sec)
        ...
        # if self.feature_type != "Pros":
        #     pass
        #     # print("[WARNING] The context padding is only implemented for Prosody features!")
        ...
        print(f"final shapes: {audio.shape}, {text.shape}")
        return audio, text